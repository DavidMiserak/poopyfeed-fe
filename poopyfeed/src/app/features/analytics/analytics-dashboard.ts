      ...
      <div class="mb-8">
        <div class="flex items-start justify-between gap-4">
          ...
          @if (!isLoading() && !error() && hasAnyData()) {
            <button
              (click)="onExportClick()"
              [disabled]="isNavigatingToExport()"
              [attr.aria-busy]="isNavigatingToExport()"
              class="flex-shrink-0 mt-2 px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg font-medium transition-colors flex items-center gap-2 disabled:bg-gray-400 disabled:cursor-not-allowed"
              aria-label="Export analytics data"
            >
              @if (isNavigatingToExport()) {
