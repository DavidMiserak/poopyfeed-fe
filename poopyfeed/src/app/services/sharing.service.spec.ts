...
describe('SharingService', () => {
  ...
  describe('createInvite', () => {
    ...
    it('should handle non_field_errors', () => {
      ...
      service.createInvite(1, createCoParentData).subscribe({
        error: (error: Error) => {
          expect(error.message).toBe('non_field_errors: Invalid invite data');
          errorCaught = true;
        },
      });
  ...
  describe('acceptInvite', () => {
    ...
    it('should handle non_field_errors', () => {
      ...
      service.acceptInvite(validToken).subscribe({
        error: (error: Error) => {
          expect(error.message).toBe('non_field_errors: You already have access to this child');
          errorCaught = true;
        },
      });
    ...
    it('should handle detail error response', () => {
      ...
      service.acceptInvite(validToken).subscribe({
        error: (error: Error) => {
          expect(error.message).toBe('Invite has been deactivated');
          errorCaught = true;
        },
      });
