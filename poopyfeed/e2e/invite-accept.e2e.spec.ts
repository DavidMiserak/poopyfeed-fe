...
test.describe('Invite Accept', () => {
  test('second user can accept invite link and see shared child', async ({ page }) => {
    const sharedChildName = await createChildAndGoToDashboard(page, 'E2E Shared');
    ...
    await expect(page.getByText(/You now have access to .+'s tracking data!/).first()).toBeVisible({
      timeout: E2E_TIMEOUT,
    });

    await page.getByRole('button', { name: 'View My Children' }).click();
    await expect(page).toHaveURL(/\/children$/, { timeout: E2E_TIMEOUT });
    await expect(page.getByRole('heading', { name: sharedChildName }).first()).toBeVisible({
      timeout: E2E_TIMEOUT,
    });
  });
  ...
  test('invalid invite token shows error', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Invalid or Expired Invite' })).toBeVisible({
      timeout: E2E_TIMEOUT,
    });
