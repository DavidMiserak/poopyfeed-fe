...
test.describe('Dashboard content', () => {
  test("dashboard shows Today's Summary section", async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: "Today's Summary", level: 2 })).toBeVisible({
      timeout: E2E_TIMEOUT,
    });
  ...
  test('dashboard shows Recent Activity after quick-logging a diaper', async ({ page }) => {
  ...
  test("Today's Summary shows diaper count after quick-log", async ({ page }) => {
    ...
    await expect(page.getByText('Diapers Today', { exact: true })).toBeVisible({
      timeout: E2E_TIMEOUT,
    });
